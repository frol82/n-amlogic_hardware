...
static const char MAC80211_MODULE_TAG[]   = "mac80211";
static const char MAC80211_MODULE_PATH[]  = "/system/lib/mac80211.ko";
static const char MAC80211_MODULE_ARG[]   = "";
#endif
...
static int is_mac80211_loaded() {
    FILE *proc;
    char line[sizeof(MAC80211_MODULE_TAG)+10];
...
+     */
    ...
    while ((fgets(line, sizeof(line), proc)) != NULL) {
        if (strncmp(line, MAC80211_MODULE_TAG, strlen(MAC80211_MODULE_TAG)) == 0) {
            fclose(proc);
            ALOGI("mac80211 driver loaded");
            return 1;
        }
    }
    fclose(proc);
...
#endif
#ifdef BOARD_WIFI_ICOMM
    if (!is_mac80211_loaded())
        if (insmod(MAC80211_MODULE_PATH, MAC80211_MODULE_ARG) < 0) {
       	ALOGE("fail to insmod mac80211 driver");
            return -1;
        }
#endif
