...
int mac80211_load_driver()
{
    if (wifi_insmod(MAC80211_MODULE_PATH, MAC80211_MODULE_ARG) !=0) {
        ALOGE("Failed to insmod  mac80211 ! \n");
        return -1;
    }
    ALOGD("Success to insmod mac80211 driver! \n");
    return 0;
}
...
int usb_wifi_load_driver()
{
    ...
    for (i = 0;i < usb_vidpid_count; i ++) {
        for (j = 0;j < sizeof(dongle_registerd)/sizeof(dongle_info);j ++) {
		 ...
		 if (!is_driver_loaded(MAC80211_MODULE_TAG))
                    mac80211_load_driver();
                }
